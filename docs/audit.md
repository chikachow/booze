...
| Medium   | The runtime dependency audit reported 14 advisories across Hono, fast-uri, ip-address, and qs.                             | These packages are updated within their existing major versions; the production dependency audit reports no advisories.          | `pnpm audit --prod --json`                                                    |
...
Preparing a previously unknown winery or site can leave an empty container if a later bottle transaction fails. Existing wine facts and bottles remain unchanged; eliminating these empty containers is a smaller follow-up to the transactional fixes. Reassigning a bottle to another vintage deliberately leaves the old vintage's reviews and awards attached to that vintage.
...
- `pnpm check`: formatting, type-aware lint, both workspace TypeScript checks, 90 Worker/script tests, 99 React tests, generated-theme validation, and 20 Chrome tests across light and dark modes. Browser coverage includes keyboard recovery, accessibility, reduced motion, 320px reflow, and large catalogues.
- `pnpm --filter @chikachow/booze-web build`, including client bundle budgets. Initial JavaScript is 134,596 bytes gzipped; total JavaScript is 210,125 bytes gzipped.
