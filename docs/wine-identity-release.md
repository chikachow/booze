...
5. Rehearse the exact candidate migrations against that export in a separate local D1 database, then test rollback/recovery in an isolated environment. Never point the rehearsal at production. From the tested checkout, use a fresh private persistence directory:

   ```sh
   umask 077
   BOOZE_REHEARSAL=$(mktemp -d)
   pnpm --filter @chikachow/booze-web exec wrangler d1 execute booze --local --persist-to "$BOOZE_REHEARSAL" --file /absolute/private/backup/booze-before-release.sql
