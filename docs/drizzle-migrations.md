...
- The application migration test applies the accepted target's SQL to disposable **local D1**, seeds a user, site, membership, wines, bottles, grape composition, and extraction evidence, then applies the candidate. It rejects changed/missing accepted SQL, checks preserved ledger records and fixture rows, checks vintage transformations and foreign keys, and compares the resulting schema with a fresh D1 database. A second application must be a no-op.
- A separate local D1 check uses the actual application Wrangler configuration and requires its applied filenames to match the entire SQL history. This catches discovery patterns that omit nested migrations.
